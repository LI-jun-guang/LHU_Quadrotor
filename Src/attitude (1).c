	...
	*/
	...
	/* 5Hz
	#define Accel_X_offset   -640.72  						//加速度計X軸偏移值
	#define Accel_Y_offset    188.27  						//加速度計Y軸偏移值
	#define Accel_Z_offset   -256.63  						//加速度計Z軸偏移值
	*/
	#define Accel_X_offset   -514								//加速度計X軸偏移值
	#define Accel_Y_offset    207							  //加速度計Y軸偏移值
	#define Accel_Z_offset    65								//加速度計Z軸偏移值
	...
	#define Gyro_dt 0.004;											//Gyro integral time
	
	#define Receiver_accleCor_Lock     R12D[8]   
	#define Receiver_accleCor_check    R12D[9]

	#define Lowpass_Filter_Gain  0.000001 
...
//		/*陀螺儀角度*/
		...
		Gyro[2].angle += (Gyro[2].new_raw-0.003)*Gyro_dt;
...
//		/*debug用*/
//		Accel[0].new_raw = IMU.Axis[0].RawData;
//		Accel[1].new_raw = IMU.Axis[1].RawData;
//		Accel[2].new_raw = IMU.Axis[2].RawData;
	}
		...
		if(!(MPU9250_Read(EXT_SENS_DATA_06)&0X08))
    {
		...
		Magn[0].new_raw = (float)(((IMU.Axis[0].RawData * MAG_ASA[0] * MAGN_Sensitivity * 10) - 166)*1.19f);
		Magn[1].new_raw = (float)(((IMU.Axis[1].RawData * MAG_ASA[1] * MAGN_Sensitivity * 10) - (-346))*0.8f);
		Magn[2].new_raw = (float)(((IMU.Axis[2].RawData * MAG_ASA[2] * MAGN_Sensitivity * 10) - 179)*1.01f);
		...
		Magn[2].angle = (atan2(my,mx)*RAD_TO_DEG)-magn_zeroPoint; 
		
		Magn[2].Lowpass_output = (Magn[2].angle * Lowpass_Filter_Gain) + (Magn[2].lastdata * (1-Lowpass_Filter_Gain));
		
		Magn[2].lastdata = Magn[2].Lowpass_output; 
		
		
//		//0~360 and -360~0 
//		if(magn_zeroPoint < 0) 
//		{
//			if((Magn[2].angle) < (180) && (Magn[2].angle) > 0) Magn[2].angle = ((-1)*(360 - Magn[2].angle)); 
//			else if((Magn[2].angle + magn_zeroPoint) > magn_zeroPoint && (Magn[2].angle + magn_zeroPoint) > 0) Magn[2].angle = Magn[2].angle - 360;
//		}
//		else if(magn_zeroPoint > 0)
//		{
//			if((Magn[2].angle+magn_zeroPoint) > (-180) && (Magn[2].angle+magn_zeroPoint) < 0)Magn[2].angle = Magn[2].angle + 360; 
//			else if( magn_zeroPoint > (Magn[2].angle + magn_zeroPoint) > 0) Magn[2].angle = 360 - Magn[2].angle;
//		}
//		if(Magn[2].angle > 360)Magn[2].angle=(360);
//		else if(Magn[2].angle <-360) Magn[2].angle=(-360);
		
//			if(magn_zeroPoint >0)
//			{
//				Magn_AngleMax = Magn_LimitMax * MAGN_LimitGain;
//				
//				Magn_AngleMin = Magn_LimitMin * MAGN_LimitGain;
//				
//				Magn[2].angle = Magn[2].angle - magn_zeroPoint;
//				
//				if(Magn[2].angle > Magn_AngleMax )Magn[2].angle = Magn_AngleMax;
//				
//				else if(Magn[2].angle < Magn_AngleMin )Magn[2].angle= Magn_AngleMin;
//			}
//			
//			if(magn_zeroPoint <0)
//			{
...
//		Magn_AngleMax = Magn_LimitMax * MAGN_LimitGain;	
//		Magn_AngleMin = Magn_LimitMin * MAGN_LimitGain;
//		Magn_AngleMax = Magn_LimitMax + magn_zeroPoint;	
//		Magn_AngleMin = Magn_LimitMin + magn_zeroPoint;
